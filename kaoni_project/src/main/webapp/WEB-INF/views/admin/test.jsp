...
<c:forEach items="${list}" var="PcRVO">
  <tr class="table-success">
  	<td id="${PcrVO.getEmnum() }" onclick="selectMember(this)">${PcrVO.getEmnum()}</td>
    <td>${PcrVO.getMemberVO().getName()}</td>
    <td>${PcrVO.getMemberVO().getPosition()}</td>
    <td>${PcrVO.getMemberVO().getDname()}</td>
    <td>${PcrVO.getIsolea()} ~ ${PcrVO.getIsolea()}</td>
    <td>${PcrVO.getPoutcome()}</td>
    <td><input type="hidden" id="emnum" name="emnum" value="">
	<input type="hidden" id="pnum" name="pnum" value="${PcrVO.getPnum()}"></td>
    </tr>
    </c:forEach>
